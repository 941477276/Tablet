...
;(function (factory) {
  ...
  function OperationRecord (options) {
    ...
    this.lineConfig = options.lineConfig ? tool.objectCopy({}, options.lineConfig) : null;
    this.bgConfig = options.bgConfig ? tool.objectCopy({}, options.bgConfig) : null;
  }
  ...
  Tablet.prototype._init = function (el, config) {
    ...
    if (config && tool.getType(config) === "object") {
      tool.objectCopy(this.config, config);
    }
      ...
      pait = function (singal) {
        switch (singal) {
          case 1:
            ...
            that.lines.push(new Line(that.ctx, that.ctxBack, tool.objectCopy({}, that.lineConfig), tool.objectCopy({}, that.bgConfig)));
          case 2:
  ...
  var tool = {
    ...
    /**
     * 对象拷贝
     * @param targetObj 目标对象或数组
     * @param sourceObj 源对象或数组
     * @returns {*}
     */
    objectCopy (targetObj, sourceObj) {
      if (Array.isArray(sourceObj)) {
        for (var i = 0, len = sourceObj.length; i < len; i++) {
          var val = sourceObj[i];
          if (typeof val == 'object' && val) {
            targetObj[i] = tool.objectCopy(Array.isArray(val) ? [] : {}, val);
          } else {
            targetObj[i] = val;
          }
        }
        ...
        for (var attr in sourceObj) {
          var val = sourceObj[attr];
          if (typeof val == 'object' && val) {
            targetObj[attr] = tool.objectCopy(Array.isArray(val) ? [] : {}, val);
          } else {
            targetObj[attr] = val;
          }
        }
